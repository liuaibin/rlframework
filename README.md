...
- ✅ **简化的算法接口** — Mixin 模式扩展 PPO、SAC、DQN，无需修改 RLlib 源码
...
# 创建报告器（用于日志记录）
reporter = FileReporter("./results/metrics.jsonl")
...
class SuccessRateCallback(FrameworkCallback):
    def on_episode_end(self, *, episode, **kwargs):
        ...
        episode.custom_metrics["success"] = float(ep_return > 195.0)
...
**完整示例：** [`examples/08_custom_model.py`](examples/08_custom_model.py)（自定义 Encoder + 自定义 Critic）
...
| Buffer | 特点 | 适用算法 |
|--------|------|----------|
| `PrioritizedSumTreeBuffer` | SumTree 优先级采样，O(log n) 复杂度 | SAC、DQN |
| `ReservoirReplayBuffer` | 水库采样，每个样本均匀保留概率 | 经验回放均衡覆盖 |
...
**完整示例：** [`examples/10_custom_training_step.py`](examples/10_custom_training_step.py)
...
reporters = [
    ...
    PrometheusReporter(gateway="localhost:9091", job="rl_training"),
]
...
class MyPPO(CustomPPO):
    ...
    def build_extra_model_config(self):
        ...
        return {
            ...
            "custom_model_config": {"hidden": 256}
        }
...
ckpt_mgr = CheckpointManager(
    save_dir="./checkpoints",
    keep_best=5,           # 保留最佳的 5 个检查点
    keep_latest=3          # 保留最新的 3 个检查点
)
...
# 加载最佳检查点
best_weights = ckpt_mgr.load_best()
```
...
### Q: 如何并行化训练？
```python
config.rollouts(num_rollout_workers=8)  # 使用 8 个并行 worker
```
