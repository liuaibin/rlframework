...
class ReservoirReplayBuffer:
    """Reservoir sampling buffer for uniform sampling guarantees.

    This buffer uses reservoir sampling to ensure each incoming sample has
    an equal probability of being retained, regardless of buffer size.
    Useful when capacity needs to be limited but all samples should have
    equal representation.

    Note:
        This class is for the **old** RLlib API stack. It is NOT compatible
        with the new EnvRunner API (Episode-based buffers). For the new stack,
        consider using RLlib's built-in `ReservoirReplayBuffer` from
    ...
    def add(self, batch: Any, **kwargs: Any) -> None:
        ...
        self._num_seen += batch.count

        if len(self._storage) < self.capacity:
            self._storage.append(batch)
        else:
            for i in range(batch.count):
                idx = random.randint(0, self._num_seen - 1)
                if idx < self.capacity:
                    if idx < len(self._storage):
                        self._storage[idx] = batch[i]
                    else:
    ...
    def sample(self, num_items: int = 32, **kwargs: Any) -> Any:
        ...
        indices = [random.randint(0, len(self) - 1) for _ in range(num_items)]
